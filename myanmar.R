...
Ytrain=as.ts(y1t[1:237])                             #define training data
Ytest=as.ts(y1t[238:245])                             #define testing data
...
ARIMA([1,2,4,5,15,17,27],2,0)(1,0,0)^7
#ARIMA model ARIMA([3,5,6,8,12,13,15],2,[2,4,5,6,7,12])
modelARIMA=arima(Ytrain, order = c(27,2,0),seasonal = list(order = c(1, 0, 0), period = 7),
...
ACF = 1,2,3,4,7,15,17,27
PACF = 1,2,3,4,13,14,17,19

ORDER MUSIMAN: AR(7) 
PACF hanya signifikan pada lag 21 -> maka PACF cut off setelah lag 21
ACF signifikan pada lag 1,2,19
Maka ACF bisa dikatakan dies down musiman dengan periode 21
Dugaan model musiman: AR(1)^21

ORDER NON MUSIMAN: 
PACF yang signifikan: lag 1,2,4,5,15,17,27
ACF yang signifikan: lag 1,2,19
Dugaan model:
1. AR([1,2,4,5,9,18,19])
2. MA([1,2,19])
