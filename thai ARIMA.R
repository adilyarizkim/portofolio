...
thai=read.csv("D:/thaicovid.csv")
thai
View(thai)
y1t <- as.ts(thai$Total.case)
#yt <- log(yt)

#READ THE DATA 7 DAYS FORECAST
Ytrain=as.ts(y1t[1:308])                             #define training data
Ytest=as.ts(y1t[309:316])                             #define testing data
...
lags <- c(12,18,24,30,36,42,48)                     #lag we used
...
ACF = 1,3,5,6,8,12,13,15,18,19,20,21,24,25,27,28
PACF = 2,4,5,6,7,12,18,21

ORDER MUSIMAN: AR(21,28) 
PACF hanya signifikan pada lag 21 -> maka PACF cut off setelah lag 21
ACF signifikan pada lag 7,21,28
Maka ACF bisa dikatakan dies down musiman dengan periode 21
Dugaan model musiman: AR(1)^21

ORDER NON MUSIMAN: 
PACF yang signifikan: lag 2,4,5,6
ACF yang signifikan: lag 1,3,5,6,8,12,13,15,18,19,20
Dugaan model:
1. AR([2,4,5,6])
2. MA([1,3,5,6,8,12,13,15,18,19,20])
